...
**DONE** 4.  **Frontend:** Add a text input for "Label Description" and a checkbox for "Extract Dates".
**DONE** 5.  **Frontend:** Pass these new configuration values in the request to `/api/extract`.
**DONE** 6.  **Frontend:** Instead of displaying raw JSON, parse the response and render the results in a user-friendly HTML table. Add a clear loading indicator that displays while waiting for the API.
